...
if [[ $FLAG != 1 ]];then
	...
	ssh $MASTER "mkdir -p /data/install/"
	scp -r  redis $MASTER:/data/install/
	ssh   $MASTER "cd /data/install/redis;sh install_instance3.sh -t install  -p '$DBPWD' -P '$DBPORT' -g '$MEMG'"
	...
	do
		...
		ssh $i "mkdir -p /data/install/" 
		scp -r  redis $i:/data/install/
		ssh   $i  "cd /data/install/redis;sh install_instance3.sh -t install  -p '$DBPWD' -P '$DBPORT' -g '$MEMG'"
		echo "[`date +"%Y-%m-%d %H:%M:%S"`] start slave install redis on $i"
	done
fi
