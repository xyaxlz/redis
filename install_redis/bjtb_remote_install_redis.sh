...
if ! expr $MEMG + 0 &>/dev/null;then
        echo "MEMG is not number"    
        exit
fi
...
if [[ $FLAG != 1 ]];then
	...
	ssh $MASTER "mkdir -p /data/install/"
	#scp -r  redis $MASTER:/data/install/
        echo  "/etc/db_tools/redis_install.py  -a '$DBPWD' -P '$DBPORT' -m '"${MEMG}"G'-r master"
	ssh   $MASTER "/etc/db_tools/redis_install.py  -a '$DBPWD' -P '$DBPORT' -m '"${MEMG}"G' -r master"
	...
	do
		...
		ssh $i "mkdir -p /data/install/" 
		#scp -r  redis $i:/data/install/
		ssh   $i  "/etc/db_tools/redis_install.py  -a '$DBPWD' -P '$DBPORT' -m '"${MEMG}"G' -r master -M ${MASTER} "
		echo "[`date +"%Y-%m-%d %H:%M:%S"`] start slave install redis on $i"
	done
fi
...
echo "[`date +"%Y-%m-%d %H:%M:%S"`] start config master-salve"
	...
	ssh  $MASTER  "echo \"slaveof NO ONE\"| /usr/local/redis-3.2.5/bin/redis-cli -a '$DBPWD' -p '$DBPORT'"
	...
	do
		...
		ssh   $i  "sed -i \"/^#slaveof/a\slaveof  ${MASTER} ${DBPORT}\"  /etc/redis/${DBPORT}.conf"
        	ssh   $i  "echo \"slaveof  ${MASTER} ${DBPORT}\"| /usr/local/redis-3.2.5/bin/redis-cli -a '$DBPWD' -p '$DBPORT'"
